...
public class BuilderUtilPackageTest {
  ...
  public void withoutAnnotation() {
    Compilation compilation =
        javac()
            .withProcessors(new ActivityBuilderProcessor())
            .compile(full(JavaFileObjects.forResource(
                "BuilderUtilsPackage/withoutAnnotation/package-info.java")));
    assertThat(compilation).succeededWithoutWarnings();
    assert_().that(compilation.generatedFiles()
        .stream()
        .filter(javaFileObject -> javaFileObject.getKind() == JavaFileObject.Kind.SOURCE).count())
        .isEqualTo(0);
    ;
  }
